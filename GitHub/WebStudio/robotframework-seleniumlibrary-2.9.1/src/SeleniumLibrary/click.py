...
class Click(RunOnFailure):
    ...
    def click_button_if_exists(self, locator):
        visible = self._selenium.is_element_present(self._parse_locator(locator))
        print (visible)
        if visible:
            self._click(self._parse_locator(locator, 'button'))
        else:
            print ('ABC')
